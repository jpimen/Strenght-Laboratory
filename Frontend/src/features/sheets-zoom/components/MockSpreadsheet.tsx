...
export const MockSpreadsheet = ({ zoomLevel }: MockSpreadsheetProps) => {
  ...
  const totalWidth = ROW_HEADER_WIDTH + columnOffsets[columnOffsets.length - 1];
  const totalHeight = COLUMN_HEADER_HEIGHT + rowOffsets[rowOffsets.length - 1];
  ...
  useEffect(() => {
    ...
    const handleMove = (event: MouseEvent) => {
      const delta = (event.clientX - columnResizeState.startX) / scale;
      setColumns((prev) =>
        prev.map((column) =>
  ...
  useEffect(() => {
    ...
    const handleMove = (event: MouseEvent) => {
      const delta = (event.clientY - rowResizeState.startY) / scale;
      setRows((prev) =>
        prev.map((row) =>
  ...
  return (
    <div className="flex flex-col h-full bg-[#f1f3f4] w-full relative border-t border-[#dadce0]">
      <div className="flex items-center gap-2 px-3 py-2 bg-white border-b border-[#dadce0]">
        <div className="text-xs text-[#5f6368] font-medium min-w-[75px]">{currentReference || "A1"}</div>
        <button
          type="button"
      ...
      <div
        ...
        className="flex-1 overflow-auto bg-[#f1f3f4] relative outline-none"
      >
        <div className="inline-block relative">
          <div className="origin-top-left shadow-lg bg-white relative" style={{ transform: `scale(${scale})` }}>
            <div className="inline-block relative bg-white text-[#3c4043] font-sans text-[13px]">
